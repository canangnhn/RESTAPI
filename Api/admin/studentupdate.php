...
if($_POST)
{
    ...
    $update = $db->db->prepare("update students set name = ?, surname = ? , email = ? ,password = ? , gender = ?, school = ?, address= ?, birth_date=?, status=? where id = ?");
    $updateResult = $update->execute(array($name,$surname,$email,$password,$gender,$school,$address,$birth_date,$status,$id));
    if($updateResult)
    {
