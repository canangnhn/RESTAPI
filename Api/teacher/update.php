...
if($_POST)
{
    ...
    $update = $db->db->prepare("update teachers set name = ?, surname = ? , email = ? , password = ? , gender = ? , birth_date= ? , school = ? , address= ? , cv= ? , lessons_id= ?, price=? where id = ?");
    $updateResult = $update->execute(array($name,$surname,$email,$password,$gender,$birth_date,$school,$address,$cv,$lessons_id,$price,$id));
    if($updateResult)
    {
